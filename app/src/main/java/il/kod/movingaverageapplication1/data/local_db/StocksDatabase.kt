...
abstract class StocksDatabase : RoomDatabase(), CoroutineScope {
    ...
    suspend fun add_db_mock(){instance?.stocksDao()?.apply {
        addStock(
            Stock(
                symbol = "AAPL",
                name = "Apple Inc.",
    ...
    private class SeedDatabaseCallback(private val database: StocksDatabase) : RoomDatabase.Callback() {
        override fun onCreate(db: SupportSQLiteDatabase) {
            super.onCreate(db)
            CoroutineScope(Dispatchers.IO).launch { database.add_db_mock() }
        }
    }
}