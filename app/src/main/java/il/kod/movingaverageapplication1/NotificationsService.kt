...
class NotificationsService : Service() {
    ...
    @Inject
    lateinit var syncManagementRepository: SyncManagementRepository

    private val binder = LocalBinder()

    //sub-service for syncing data with the remote server
    val syncing: Syncing = Syncing()

    //sub-service for notifications
     val notifications: Notifications = Notifications()

    var notifiableFollowSets: MutableList<FollowSet>? =
        null // This will be the list of follow sets that we want to notify the user about
    ...
    interface OnReadyListener {
        fun onServiceReady(service: NotificationsService)
    }
    ...
    inner class LocalBinder : Binder() {
        fun getService(): NotificationsService = this@NotificationsService
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        serviceScope.launch {
            ...
            tempImmutableFetch.let {
                notifiableFollowSets = it.toMutableList()
            }// Convert to MutableList if not null
            notifiableFollowSets?.let {
                Log.d("NotificationService", "Detected Notifiable Follow Sets: ${it.size}")
                it.forEach { followSet ->
                    Log.d(
                        "NotificationService",
                        "Monitoring Follow Set: ${followSet.name} with threshold ${followSet.notificationsPriceThreeshold}"
                    )
    ...
    fun detectAndNotifyUser(followSet: FollowSet) {
        Log.d(
            "NotificationService",
            "Starting to monitor Follow Set: ${followSet.name} of size ${followSet.set_ids.size} with threshold ${followSet.notificationsPriceThreeshold}"
        )
    ...
    inner class Syncing {
        val isSomethingDirty: Boolean = false


        fun syncDirtyData()
        {
//            syncManagementRepository.pushFollowSetToRemoteDB()
